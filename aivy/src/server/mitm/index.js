...
/**
 * Request format : ORIGINAL xxx.xxx.xxx.xxx:xxxxx xxxx
 *                  ORIGINAL IP:PORT PROCESSID
 * 
 ...
 * @param {_ => void} client_close_handler 
 */
const mitm = (
    ...
    // set listening event
    server.on('listening', () => server_listening_handler(server));
    // set close event
    ...
    server.on('connection', socket => {
            ...
            if (data_string.startsWith('ORIGINAL')) {
                ...
                socket.remote.on('data', remote_data => {
                    ...
                    if (remote_data.length > 0 && !socket.is_closing && socket.remote.is_connected) {
                        client_data_handler(server, socket, remote_data, false)
                            .then(buffer => {
                                try {
                ...
                if (data.length > 0 && !socket.is_closing && socket && socket.remote.is_connected) {
                    client_data_handler(server, socket, data, true)
                        .then(buffer => {
                            try {
                                if (buffer.length > 0) socket.remote.write(buffer);
                                else socket.remote.destroy();
                            } catch (e) {
                                console.error(e);
                            }
                        });
        ...
        client_connection_handler(server, socket);
    });
    ...
    server.start = (port) => {
        ...
        // add sigint close
        process.addListener('SIGINT', sigint_close);
    }
    ...
    server.stop = () => {
        ...
        process.removeListener('SIGINT', sigint_close);
    }
